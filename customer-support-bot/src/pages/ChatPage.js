...
function ChatPage() {
  ...
  const fetchChatHistory = async () => {
    try {
      const token = localStorage?.getItem('token');
      if (!token) {
        // For demo purposes, show sample data when no token
        const mockHistory = [
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // For demo purposes when localStorage is not available
      if (error.message.includes('localStorage')) {
        // Simulate bot response for demo
        await new Promise(resolve => setTimeout(resolve, 1000));
        const botResponse = `Thank you for your question: "${currentQuestion}". This is a demo response since we can't access your backend in this artifact environment.`;
        const botMessage = { sender: 'bot', message: botResponse };
        setChatHistory(prev => [...prev, botMessage]);
        setConversationId(Date.now().toString());
      } else {
        // Show error message to user
        const errorMessage = { sender: 'bot', message: 'Something went wrong. Please try again.' };
